...
func ClientSideGetRobotList(c *gin.Context) {
	...
	list, err := msql.Model(`chat_ai_robot`, define.Postgres).
		...
		Where(`is_deleted`, cast.ToString(define.Normal)).
		Where(`application_type`, `0`).
		Order(`is_top desc, sort_num desc, id desc`).Select()
	if err != nil {
		logs.Error(err.Error())
		c.String(http.StatusOK, lib_web.FmtJson(nil, errors.New(i18n.Show(common.GetLang(c), `sys_err`))))
		return
	...
	defaultRobotKey := ``
	if len(list) > 0 {
		defaultRobotKey = cast.ToString(list[0][`robot_key`])
	}
	data := map[string]any{
		`h5_domain`:         define.Config.WebService[`h5_domain`],
		`list`:              list,
		`company`:           company,
		`default_robot_key`: defaultRobotKey,
	}
