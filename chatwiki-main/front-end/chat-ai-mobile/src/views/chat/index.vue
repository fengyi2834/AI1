...
const getDetectedLang = (): string | null => {
  ...
  // 3. externalConfigH5 配置
  const externalLang = chatStore.externalConfigH5.lang
  if (externalLang && supportedLangs.includes(externalLang)) {
    return externalLang
  }
  // 4. 浏览器语言
...
// 首次加载时根据 URL 参数 / localStorage / externalConfigH5 / 浏览器语言 自动切换
const initialDetectedLang = getDetectedLang()
if (initialDetectedLang && initialDetectedLang !== localeStore.currentLocale.lang) {
  changeLocale(initialDetectedLang as LocaleType)
}

// 监听 URL ?lang 参数变化，自动切换语言
watch(
  () => route.query.lang,
  async (newLang) => {
    if (newLang && typeof newLang === 'string' && newLang !== localeStore.currentLocale.lang && supportedLangs.includes(newLang)) {
...
watch(
  ...
  async (newLang) => {
    // URL 参数的优先级高于 externalConfigH5，所以如果 URL 已经设置了 lang 则不覆盖
    const urlLang = route.query.lang as string
    if (urlLang && supportedLangs.includes(urlLang)) {
      return
    }
    if (newLang && newLang !== localeStore.currentLocale.lang && supportedLangs.includes(newLang)) {
      await changeLocale(newLang as LocaleType)
    }
  },
  { immediate: true }
